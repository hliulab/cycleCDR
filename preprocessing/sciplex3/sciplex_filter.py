...
adata: AnnData = ad.concat(adatas, join="outer", label="batch", index_unique="-")
...
# 剔除 adata 中基因表达大于 0 的个数小于 500 的样本
print("剔除 adata 中基因表达大于 0 的个数小于 500 的样本")
print(adata.X.shape)
print(adata.obs.shape)
adata = adata[adata.X.getnnz(axis=1) >= 500, :]
print(adata.X.shape)
print(adata.obs.shape)

# 剔除 adata 中基因表达大于 0 的次数小于 1000 的基因
print("剔除 adata 中基因表达大于 0 的次数小于 1000 的基因")
print(adata.X.shape)
print(adata.var.shape)
adata = adata[:, adata.X.getnnz(axis=0) >= 1000]
print(adata.X.A.shape)
print(adata.var.shape)
...
sc.write("./datasets/preprocess/sciplex3/sciplex3_filtered_genes.h5ad", adata)
