...
from preprocessing.adamson.helper import rank_genes_groups_by_cov


adata = sc.read_h5ad("./datasets/row/adamson/perturb_processed.h5ad")
...
sc.pp.highly_variable_genes(adata, n_top_genes=1421, subset=False)
...
deg_gene.to_csv("./datasets/preprocess/adamson/deg_gene.csv")

adata.uns["rank_genes_groups_cov_all"] = deg_dict

adata.write_h5ad('./datasets/preprocess/adamson/pre_perturb_processed.h5ad')

pert_index = {}
for t in pert_symbol_to_enst.keys():
    pert_index[t] = adata.var[adata.var['gene_name'] == t]["position"].to_numpy()[0]

with open('./datasets/preprocess/adamson/pert_index.pkl', 'wb') as f:
...
with open("./datasets/preprocess/adamson/mse_hvg_idx.pkl", "wb") as f:
