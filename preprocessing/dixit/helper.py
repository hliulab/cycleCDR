...
def rank_genes_groups_by_cov(
    ...
    for cov_cat in cov_categories:
        ...
        adata_control = adata_cov[adata_cov.obs['control'] == 1].copy()
        adata_treat = adata_cov[adata_cov.obs['control'] == 0].copy()

        control = adata_control.X.A.mean(axis=0)

        adata_treat_df = adata_treat.to_df()
        adata_treat_df[groupby] = adata_treat.obs[groupby]

        for cond, df in adata_treat_df.groupby(groupby, observed=False):
